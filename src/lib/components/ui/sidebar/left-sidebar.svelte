<script lang="ts">
	...
	import { type Keyword, type KeywordMap, type TreeNodeData, loadData } from '$lib';
	...
	import { getKeywordsForNode, mergeKeywords } from '$lib/utils';
	...
	$: {
		...
		keywords = mergeKeywords(nodess.map((node) => getKeywordsForNode(node)));
	}
		...
		searchResults = Object.entries(nodes)
			.filter(
				([_, values]) =>
					...
					values.skill?.name.toLowerCase().includes(search) ||
					values.skill?.description.some((value) => value.toLowerCase().includes(search))
			)
			.map(([key, _]) => key);
	}
...
<aside
	class={`h-full grid grid-cols-1  ${sidebarVisible ? 'bg-[#111]' : 'absolute'} grid-rows-[auto_auto_auto_1fr] gap-2 p-2  min-h-0 z-20`}
>
	...
	>
		<h2 class="-mt-1 text-2xl">{sidebarVisible ? '<' : '>'}</h2>
	</button>
	{#if sidebarVisible}
		<!-- Toggleable -->
		<div class="space-y-4">
